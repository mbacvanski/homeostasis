"""H68b: GA followers onto synthetic circles (correct chirality) at two radii."""
...
def circle(r, n=7200):
    om = np.deg2rad(-1.92)
    t = np.arange(n)
    return np.stack([15.0 + r * np.cos(om * t), 15.0 + r * np.sin(om * t)], axis=1)
TRAJS = {"c934": circle(9.34), "c706": circle(7.06)}
...
def main():
    ...
    (LAB / "h68b_circles.json").write_text(json.dumps(out))
