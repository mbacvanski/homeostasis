"""H101-rev: jitter introduced AFTER lock formation."""
...
def run(task):
    ...
    for i in range(n):
        ...
        amp = min(max(i - 3600, 0) / 500.0, 1.0)
        px, py = LOOP[i]
        env.sx = float(px) + amp * jx
        env.sy = float(py) + amp * jy
        dist[i] = env.distance()
        st = net.step(env.sense())
        env.apply_action(*map(float, st.outputs))
        env.steps += 1
    return dict(sd=sd, tau=tau, jseed=jseed,
                near4=float((dist[4100:] < 4).mean()))
...
def main():
    ...
    (LAB / "h101rev_postlock.json").write_text(json.dumps(rows))
    for sd, tau in cells:
