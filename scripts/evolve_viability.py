"""Evolve tracking-model hyperparameters on PURE INTERNAL VIABILITY.

Selection never sees behavior; the within-45 tracking score is recorded for
every evaluation but never used. The experiment asks whether tracking emerges
as a byproduct of selecting bodies whose neurons stay viable (the paper's own
evolutionary conjecture, discussion p. 1831).

Viability is defined on the ESSENTIAL VARIABLE — each node's firing rate —
not on the homeostatic error |E|. Fitness maximizes the fraction of
(node, 120-step window) pairs whose firing rate lies in a viable band
(0.05 to 0.9): silence starves, saturation burns. The first version of this
experiment used mean |E| and evolution found the degenerate optimum within
two generations (see scripts/out/evolution/naive_meanE_arm.log): with every
node spiking every step, the network reaches a fixed point x' = I/(leak+2)
and the *adaptive targets inflate to match it*, driving |E| to ~0.009 while
the agent sits frozen at chance. Homeostatic satisfaction is not viability
when the set-point itself can drift to bless any state; the band on firing
rate is the physiological quantity the paper's own homeostasis citations
(O'Leary & Wyllie; Turrigiano) describe, and target inflation cannot game it.
mean |E| is still recorded for every evaluation.

Genome: the 9 scalar model parameters (local node properties + effector gain).
GA: tournament selection, uniform crossover, log/linear Gaussian mutation,
elitism, evaluation seeds RESAMPLED each generation so lineages cannot
overfit a lucky wiring draw.

Outputs into scripts/out/evolution/:
  log lines per generation, evolution.json (every evaluation ever), 4 PNGs,
  champions.json (top configs, loadable by the visualizer).

Usage: python scripts/evolve_viability.py [--generations 30] [--pop 28]
"""
...
def evaluate(task):
    ...
    sim = TrackingSimulation(
        ReservoirConfig(**r_kwargs), TrackingConfig(gain=genome["gain"]), seed=seed
    )
    net = sim.network
    ...
    return {
        "viability": 1.0 - rate_band,            # inviability (lower = fitter)
        "rate_band": rate_band,                  # fraction of node-windows viable
        "starving": starve_sum / n_windows,      # rate <= RATE_LO
        "saturated": saturate_sum / n_windows,   # rate >= RATE_HI
        "mean_abs_E": sum_abs_e / count,         # the old (gameable) criterion
...
def main():
    ...
    ap.add_argument("--out", type=str, default="scripts/out/evolution")
    args = ap.parse_args()
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        ...
        for rep in range(args.replicates):
            ...
            for gen in range(args.generations):
                ...
                hist.append(gen_row)
                print(f"rep{rep} gen{gen:02d} | viable-band best {best['rate_band']:.3f} "
                      f"pop {1 - gen_row['mean_viability']:.3f} | score best-fit {best['score']:.2f} "
                      f"popmax {gen_row['max_score']:.2f} | spike {best['prop_spiked']:.2f} "
                      f"starve {best['starving']:.2f} sat {best['saturated']:.2f} "
        ...
        for label, h in zip([f"champion rep{r}" for r in range(len(champions))] + ["paper"], held):
            print(f"  {label:>14}: viable-band {h['rate_band']:.3f} | score {h['score']:.3f} "
                  f"± {h['score_sd']:.3f} | dir {h['dir_agree']:.2f} | "
                  f"spike {h['prop_spiked']:.2f} | mean|E| {h['mean_abs_E']:.3f}", flush=True)
        ...
        for condition in ("full", "no-learn", "lesion"):
            ...
            autopsy[condition] = r
            print(f"  {condition:>9}: score {r['score']:.3f} | viable-band {r['rate_band']:.3f} "
    ...
    for r, (g, h) in enumerate(zip(champions, held[:-1])):
        champ_entries.append({
            "id": f"evo{r}",
            "label": f"viability champion rep{r}",
...
def make_plots(history, evals, held, finalists, out):
    ...
    ax1.set_ylabel("1 − viable-band fraction (fitness, lower = better)")
    ax1.set_title("What selection sees: firing-rate viability")
    ...
    fig.suptitle("Evolution on pure internal viability (score recorded, never selected on)")
    fig.tight_layout()
    ...
    ax.set_xlabel("1 − viable-band fraction (selection minimizes this)")
    ...
    for r in reps:
        ...
        ax.plot(gens, [row["best_in_band"] for row in h], color=colors[r], lw=1.2,
                ls="--", label=f"rep {r} viable band [{RATE_LO},{RATE_HI}]")
        ax.plot(gens, [row["best_starving"] for row in h], color=colors[r], lw=1.0,
                ls=":", label=f"rep {r} starving (rate<{RATE_LO})")
