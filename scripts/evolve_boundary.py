"""Evolve tracking-model hyperparameters on BOUNDARY VIABILITY.

Fitness lives on the sensory surface: the fraction of (sensor, 120-step
window) pairs whose mean activation sits in the viable band (0.05 to 0.9).
Sensors are stateless and clamped by world geometry, so no internal dynamics
can satisfy this criterion - the two interior-viability cheats (set-point
inflation, the autarkic recurrent hum; see evolve_viability.py) are excluded
by construction. Per-sensor banding also punishes perfect fixation (the
foveal sensors saturate while the rest starve), predicting oscillatory
"active sensing" rather than dead-lock pursuit. Interior rate-band, score,
and turn magnitude are recorded for every evaluation but never selected on.
...
Outputs into scripts/out/evolution_boundary/:
...
Usage: python scripts/evolve_boundary.py [--generations 30] [--pop 28]
"""
...
def evaluate(task):
    ...
    return {
        "viability": 1.0 - sensor_band,          # BOUNDARY inviability (fitness)
...
def main():
    ...
    args = ap.parse_args()
    out = pathlib.Path(args.out or f"scripts/out/evolution_boundary_{args.aperture}")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        ...
        for rep in range(args.replicates):
            ...
            for gen in range(args.generations):
                ...
                hist.append(gen_row)
                print(f"rep{rep} gen{gen:02d} | sens-band best {best['sensor_band']:.3f} "
        ...
        for label, h in zip([f"champion rep{r}" for r in range(len(champions))] + ["paper"], held):
            print(f"  {label:>14}: sens-band {h['sensor_band']:.3f} | score {h['score']:.3f} "
        ...
        for condition in ("full", "no-learn", "lesion"):
            ...
            autopsy[condition] = r
            print(f"  {condition:>9}: score {r['score']:.3f} | sens-band {r['sensor_band']:.3f} "
    ...
    for r, (g, h) in enumerate(zip(champions, held[:-1])):
        champ_entries.append({
            "id": f"evoB-{args.aperture}-{r}",
            "label": f"boundary {args.aperture} champion rep{r}",
            "metrics": {"score": round(h["score"], 3),
                        "dir_agree": round(h["dir_agree"], 2),
                        "prop_spiked": round(h["prop_spiked"], 2),
                        "sensor_band": round(h["sensor_band"], 3),
...
def make_plots(history, evals, held, finalists, out):
    ...
    ax1.set_ylabel("1 − sensor-band fraction (fitness, lower = better)")
    ax1.set_title("What selection sees: boundary (sensor) viability")
    ...
    fig.suptitle("Evolution on boundary (sensor) viability - score recorded, never selected on")
    fig.tight_layout()
    ...
    ax.set_xlabel("1 − sensor-band fraction (selection minimizes this)")
    ...
    for r in reps:
        ...
        ax.plot(gens, [row["best_in_band"] for row in h], color=colors[r], lw=1.2,
                ls="--", label=f"rep {r} sensor band [{RATE_LO},{RATE_HI}]")
