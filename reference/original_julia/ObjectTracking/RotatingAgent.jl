...
#sensory_nodes = StatsBase.sample(range(1,nnodes),Weights(repeat([1],nnodes)),Int(round(nnodes*.3)))
...
ax_sensors = Axis(sensors_layout[1,1];
    backgroundcolor = :lightgrey, title = "Sensor Activations", titlefontsize = 32, ylabel = "Value")
...
ax_effectors = Axis(effectors_layout[1,1];
    backgroundcolor = :lightgrey, title = "Effector Activations", titlefontsize = 100, ylabel = "Value", xticks = (1:2, ["Left Turn", "Right Turn"]))
xlims!(ax_effectors, (0,3))
ylims!(ax_effectors, (0,1))

ax_weights = Axis(weights_layout[1,1]; backgroundcolor = :lightgrey, title = "Weight Distribution")
