...
#using Agents, LinearAlgebra, Statistics, Meshes, Distributions, DataFrames, StatsBase, Random
...
function get_input_acts(stim,agent)
    ...
    ang = rad2deg(angle(stim.complexPos - agent.complexPos))
    angDists = abs.(sens_degrees .- ang)
    stimDist = 1-abs(stim.complexPos-agent.complexPos)/900
    # if stimDist < .5
    #     stimDist = .5
    # end

    sens_acts = float(angDists .< 2) #.* stimDist
